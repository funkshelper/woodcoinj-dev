...
public abstract class AbstractBlockChain {
    ...
    public boolean add(Block block) throws VerificationException, PrunedException {
        try {
            ...
            log.warn("Exeption caught HOLMES in add(Block block) in abstractBlockChain");
            try {
                notSettingChainHead();
    ...
    private boolean add(Block block, boolean tryConnecting,
       ...
       //log.warn("ABC add point 1");
        //System.out.println("What is going on G!!  HELP");
        lock.lock();
        try {
            ...
            //log.warn("ABC add point 2");
	    if (tryConnecting && orphanBlocks.containsKey(block.getHash())) {
                return false;
            }
            //log.warn("ABC add point 33");
            ...
            if (shouldVerifyTransactions() && block.transactions == null) {
                log.warn("Hey wait a minute.. should verify transactions is on??  got a header while running in full mode");
                throw new VerificationException("Got a block header while running in full-block mode");
	    }

            //log.warn("ABC add point 4");
		...
		//log.warn("prevblockhash at this juncture: " + block.getPrevBlockHash());
                ...
                if (storedPrev != null) {
                    ...
                    log.warn("and... storedPrev was null");
    ...
    private void connectBlock(final Block block, StoredBlock storedPrev, boolean expensiveChecks,
        ...
        if (storedPrev.equals(head)) {
          //  log.warn("inside cB4 storedPrev equals head");
            ...
            log.warn("cb5 block connects somewhere else");
